...
The complete production-controls walkthrough—including authentication, provider/temperature/token policy, Reader/Reviewer authorization, separate approval, prompt-injection checks, telemetry, and the `MaxConversationTurns` coding challenge—is in [LAB.md](LAB.md). The runnable curl commands formerly in this README are preserved there.
